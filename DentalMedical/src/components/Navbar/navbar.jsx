  ...
  return (
    ...
    <nav className='flex flex-row justify-between h-max mt-10 mb-24 items-center md:bg-[#E6F6FE] md:rounded-[0.63rem] md:mb-20 md:px-8 md:py-5'>
      ...
      <div id='panigator' className=' hidden md:flex md:flex-row md:justify-center md:mx-auto md:gap-10'>
        ...
        <div key={data.id}>
          <Link to={data.to} onClick={()=>handleLinkClick(data.id)} className={`transition-all text-lg duration-200 hover:font-bold ${activeLink === (data.id) ? ('font-bold'):('font-normal')}`}>{data.desc}</Link>
        </div>
        ))}
      </div>
